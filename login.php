...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $mysqli->prepare("SELECT id, password, usr_type FROM users WHERE email = ?");
    if ($stmt) {
        ...
        if ($stmt->num_rows == 1) {
            $stmt->bind_result($id, $hashed_password, $usr_type);
            $stmt->fetch();
            ...
            if (password_verify($password, $hashed_password)) {
                ...
                header("location: index.html"); // Redirect to dashboard or home
                exit();
            } else {
